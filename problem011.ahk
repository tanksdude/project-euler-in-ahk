...
; Problem #11 text:
; In the 20×20 grid below, four numbers along a diagonal line have been marked in red. [Look at the actual text for the coloring.]
...
gridLength := 20 ; it's square

NUM := 4 ; this is "supposed" to be a global constant but it doesn't affects very much; the less I can do in AHK to solve these problems, the better
answer := 0
...
{
	x := 1
	while (x <= gridLength - NUM)
	{
		prod := numGrid[y][x] * numGrid[y][x+1] * numGrid[y][x+2] * numGrid[y][x+3]
		if (prod > answer) {
			answer := prod
		}
		x := x + 1
...
; second: vertical
y := 1
while (y <= gridLength - NUM)
{
	x := 1
	while (x <= gridLength)
	{
		prod := numGrid[y][x] * numGrid[y+1][x] * numGrid[y+2][x] * numGrid[y+3][x]
		if (prod > answer) {
			answer := prod
		}
		x := x + 1
...
while (y <= gridLength - NUM)
{
	...
	while (x <= gridLength - NUM)
	{
		prod := numGrid[y][x] * numGrid[y+1][x+1] * numGrid[y+2][x+2] * numGrid[y+3][x+3]
		if (prod > answer) {
			answer := prod
		}
		x := x + 1
